...
char2 = Unit(2,'ally2',fighter,[aburnmod],level=5,player_id=3)
...
for x in char2.get_action_modifiers():
    x.resolve_affliction(char2.combat_stats,char3.combat_stats,char3.buffs)
for x in char3.get_turn_modifiers():
    print(x)
##Getting a bit ridiculous... Maybe create a wrapper class for modifiers that is just a list of modifiers
""" for x in char2.get_action_modifiers():
    x.resolve_affliction(char2.combat_stats,char3.combat_stats,char3.buffs)
    for x in char3.get_turn_modifiers():
